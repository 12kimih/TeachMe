...
def main():
    ...
    parser.add_argument(
        "--eval-model",
        help="Model for evaluating report quality (default: openai:gpt-4-turbo)",
    )
    ...
    for agent in agents_to_test:
        ...
        cmd = ["python", "-m", "pytest", test_file] + pytest_options + [f"--research-agent={agent}"]
        ...
        if "POTENTIAL IRRELEVANT SECTIONS DETECTED" in stdout:
            ...
            section_analysis = re.search(
                r"⚠️ POTENTIAL IRRELEVANT SECTIONS DETECTED:.*?(?=\n\n|\Z)",
                stdout,
                re.DOTALL,
            )
            if section_analysis:
