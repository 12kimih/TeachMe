...
from open_deep_research.utils import duckduckgo_search, get_config_value, get_today_str, load_mcp_server_config, tavily_search
...
class Question(BaseModel):
    ...
    question: str = Field(description="A specific question to ask the user to clarify the scope, focus, or requirements of the report.")
...
async def supervisor_tools(state: ReportState, config: RunnableConfig) -> Command[Literal["supervisor", "research_team", "__end__"]]:
    ...
    search_tool_names = {tool.name for tool in supervisor_tool_list if tool.metadata is not None and tool.metadata.get("type") == "search"}
    ...
    return {
        "messages": [
            ...
            await llm.bind_tools(
                ...
                # force at least one tool call
                tool_choice="any",
            ).ainvoke([{"role": "system", "content": system_prompt}] + state["messages"])
        ]
    }
...
async def research_agent_tools(state: SectionState, config: RunnableConfig):
    ...
    search_tool_names = {tool.name for tool in research_tool_list if tool.metadata is not None and tool.metadata.get("type") == "search"}
...
research_builder.add_conditional_edges("research_agent", research_agent_should_continue, ["research_agent_tools", END])
...
supervisor_builder.add_conditional_edges("supervisor", supervisor_should_continue, ["supervisor_tools", END])
